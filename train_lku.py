...
from models.LKU import LKU
from models.LKU import SpatialTransform
...
parser.add_argument('--train_dir', type=str, default='../../data/IXI_data/Train/')
parser.add_argument('--val_dir', type=str, default='../../data/IXI_data/Val/')
parser.add_argument('--label_dir', type=str, default='../../data/OASIS/label/')
parser.add_argument('--dataset', type=str, default='IXI')
parser.add_argument('--atlas_dir', type=str, default='../../data/IXI_data/atlas.pkl')

parser.add_argument('--model', type=str, default='LKU')
...
parser.add_argument('--weight_model', type=float, default=5.0)
parser.add_argument('--weight_opt', type=float, default=5.0)
...
def load_model(img_size):
    if args.model == 'TransMorph':
        ...
        model = LKU(2, 3, args.start_channels) # start_channels = 8
    model.cuda()
...
def main():
    ...
    '''
    reg_model = SpatialTransform() if args.model == 'LKU' else utils.register_model(img_size, 'nearest')
    reg_model.cuda()
    ...
    criterion_reg = losses.smoothloss if args.model == 'LKU' else losses.Grad3d(penalty='l2')
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        for data in train_loader:
            ...
            if optron_epoch:
                ...
                for i in range(args.optron_epoch):
                    ...
                    optron_loss_reg = criterion_reg(optimized_flow) * weights_opt[1] if args.model == 'LKU' else criterion_reg(optimized_flow, y) * weights_opt[1]
                ...
                loss_reg = criterion_reg(output[1]) * weights_model[1] if args.model == 'LKU' else criterion_reg(output[1], y) * weights_model[1]
                loss = loss_mse + loss_reg
                loss_vals = [loss_mse, loss_reg]
                loss_all.update(loss.item(), y.numel())
            else:
                loss_ncc = criterion_ncc(output[0], y)
                loss_reg = criterion_reg(output[1]) * weights_model[1] if args.model == 'LKU' else criterion_reg(output[1], y)
            ...
            if args.dataset == "OASIS":
                ...
                if optron_epoch:
                    ...
                    for i in range(optron_epoch):
                        ...
                        optron_loss_reg = criterion_reg(optimized_flow) * weights_opt[1] if args.model == 'LKU' else criterion_reg(optimized_flow, x) * weights_opt[1]
                        optron_loss = optron_loss_ncc + optron_loss_reg
                    ...
                    loss_reg = criterion_reg(output[1]) * weights_model[1] if args.model == 'LKU' else criterion_reg(output[1], x) * weights_model[1]
                    loss = loss_mse + loss_reg
                    loss_vals = [loss_mse, loss_reg]
                else:
                    loss_ncc = criterion_ncc(output[0], x)
                    loss_reg = criterion_reg(output[1]) * weights_model[1] if args.model == 'LKU' else criterion_reg(output[1], x)
                    loss = loss_ncc + loss_reg
                    loss_vals = [loss_ncc, loss_reg]
        ...
        with torch.no_grad():
            for data in val_loader:
                ...
                x_in = torch.cat((x, y), dim=1)
                output = model(x_in)
                def_out = reg_model(x_seg.cuda().float(), output[1].permute(0, 2, 3, 4, 1).cuda(), mod='nearest') if args.model == 'LKU' else reg_model([x_seg.cuda().float(), output[1].cuda()])
